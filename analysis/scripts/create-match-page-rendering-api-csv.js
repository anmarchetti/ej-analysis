...
const csvWriter = createCsvWriter({
    path: 'output_data/RenderingApiRelations.csv',
    header: [
      {id: 'page', title: 'Page'},
  ...
  for (const page of pages) {
    ...
    try {
      const result = await session.run(
        "MATCH p=(page:Page {webUrl: $pageUrl})-[*..4]-(rendering:Rendering)-[r:HAS_FUNCTION]->(c)-[ca:CALLS*..10]->(f:Function {owner: 'endpoints.ts'})-[ap:CALLS_API]->(api: APIInterface) RETURN DISTINCT page, rendering, api, count(api) as count",
        { pageUrl: page }
      );
