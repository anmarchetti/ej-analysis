...
const exclusions = ['.test.', 'componentFactory', 'icons', 'createMockStores', 'src/backend']
const skipFunctions = [];
...
if (fs.existsSync(logFilePath3)) {
  fs.unlinkSync(logFilePath1);
  fs.unlinkSync(logFilePath2);
  fs.unlinkSync(logFilePath3);
}
      ...
      node.members.forEach(member => {
        ...
        // handle arrow functions or function expressions assigned to properties
          ...
          if (traceComponent && fileName == traceComponent && (node.name || node.initializer)) {
            logger.info(`Node ${member.name?.text} kind: ${ts.SyntaxKind[member.kind]}, isFunctionExpression: ${ts.isFunctionExpression(member.initializer)}, isArrowFunction: ${ts.isArrowFunction(member.initializer)}`);
          }
...
function getImportMappings(sourceFile, filePath, baseDir) {
  ...
  ts.forEachChild(sourceFile, node => {
    if (ts.isImportDeclaration(node) && node.importClause) {
      ...
      if (fs.existsSync(resolvedPath)) {
        const imp = path.basename(resolvedPath);
...
function getMobxInjectedMethods(filePath, baseDir) {
  const content = fs.readFileSync(filePath, 'utf8');
  ...
  ts.forEachChild(sourceFile, node => {
    //if (ts.isExportAssignment(node) && node.expression.expression)
    //logger.error(ts.isExportAssignment(node) + ' ' +  ts.isCallExpression(node.expression) + ' ' + node.expression.expression.getText(sourceFile))
    if (ts.isExportAssignment(node) && ts.isCallExpression(node.expression) && node.expression.expression.getText(sourceFile).startsWith('inject')) {

      const injectArguments = node.expression.expression.arguments;
            ...
            if (ts.isObjectLiteralExpression(injectFunction.body.expression)) {
              injectFunction.body.expression.properties.forEach(prop => {
                if (ts.isPropertyAssignment(prop)) {
                  ...
                  // Managing for ex. method stores.layoutStore.isShortlistPage
                  const mobxStoreMethodElements = prop.initializer.getText(sourceFile).split('.')

                  if (mobxStoreMethodElements.length > 0)
                    mobxStoreMethod = mobxStoreMethodElements[mobxStoreMethodElements.length - 1];

                  mobxInjectedMethods[methodName] = mobxStoreMethod;
                }
              });
            }
          }
...
function extractFunctionCalls(fileName, filePath, baseDir) {
  ...
  const mobxInjectedMethods = getMobxInjectedMethods(filePath, baseDir);

  function visit(node) {

    if (traceComponent && fileName == traceComponent) {
      printNodeInformation(node, sourceFile)
    }

    // if (ts.isCallExpression(node) && traceComponent && fileName == traceComponent) {
    //   logger.info(`Case 0: Handle function declarations - Node %o`, node.expression?.getText(sourceFile))
    ...
    if (ts.isFunctionDeclaration(node) || ts.isMethodDeclaration(node) ||
      ...
      (ts.isVariableDeclaration(node) && node.initializer && ts.isFunctionLike(node.initializer))
    ) {
      if (traceComponent && fileName == traceComponent && (node.name || node.initializer)) {
        logger.info(`Case 1: Handle function declarations - Node ${node.name?.text}`)
      }
    ...
    // Case 2: Handle class declaration
      ...
      if (traceComponent && fileName == traceComponent && (node.name || node.initializer)) {
        logger.info(`Case 2: Handle class declaration -  Node ${node.name?.text}`)
      }
      node.members.forEach(visit);
    }
    // Case 3: Handle property declaration or functionlike
    else if (ts.isPropertyDeclaration(node) && node.initializer && ts.isFunctionLike(node.initializer)) {
      if (traceComponent && fileName == traceComponent && (node.name || node.initializer)) {
        logger.info(`Case 3: Handle property declaration or functionlike - Node ${node.name?.text}`)
      }
    ...
    // Case 4: Handle JsxElement
      ...
      if (traceComponent && fileName == traceComponent && (node.name || node.initializer)) {
        logger.info(`Case 4: Handle JsxElement - Node ${node.name?.text}`)
      }
    ...
    // Case 5: Handle JsxExpression
      ...
      if (node.expression && ts.isCallExpression(node.expression)) {
        ...
        recordFunctionCallInContext(node.expression, 'JSX', fileName);
      }
    }
    //Case 6: Handle store function assignment
    else if (node.name && node.initializer?.name && ts.isPropertyAssignment(node) &&
      ts.isPropertyAccessExpression(node.initializer) && checkFunctionFromImportedStore(node.initializer?.name?.text, importMappings)) {

      if (traceComponent && fileName == traceComponent && (node.name || node.initializer)) {
        logger.info(`Case 6: Handle store function assignment - Node ${node.name?.text} - Initializer ${node.initializer?.name.text}`)
      }

      const store = checkFunctionFromImportedStore(node.initializer?.name.text, importMappings);

      if (traceComponent && fileName == traceComponent) {
        logger.info(`Case 6: Found store function binding and the store is ${store}`);
      }

      const caller = node.name?.text;
      const callee = node.initializer?.name?.text;
      const owner = store;
      ...
      const callDetail = { owner: owner, callName: callee };
      functionCalls[caller].add(callDetail);

      logger.info(`Case 6: storing {caller: ${caller}, owner: ${owner}, function: ${callee} }`)
    }
    //Case 7: Handle MobX injected methods usage
    else if (ts.isIdentifier(node) && mobxInjectedMethods[node.name?.text]) {
      if (traceComponent && fileName == traceComponent && (node.name || node.initializer)) {
        logger.info(`Case 7: Mobx injections - Node ${node.name?.text} - Initializer ${node.initializer?.name.text}`)
      }

      const mobxMethodCall = mobxInjectedMethods[node.name?.text].split('.');
      const methodName = mobxMethodCall.pop();
      const storeName = mobxMethodCall.join('.');

      functionCalls[defaultCaller].add({ owner: storeName, callName: methodName });

      //logger.info(`extractFunctionCalls (MobX): storing {caller: ${defaultCaller}, owner: ${storeName}, method: ${methodName} }`);
    }
    else if (ts.isPropertyAccessExpression(node)
      && node.expression && node.expression?.getText(sourceFile) == 'props'
      && node.parent && ts.isCallExpression(node.parent)) {

      const func = node.name?.text;
      const owner = fileName;
      const caller = findParentCallerFunction(node.parent)?.name?.text

      if (traceComponent && fileName == traceComponent && (node.name || node.initializer)) {
        logger.info(`Case 8: Props functions - Node ${node.name?.text} - Initializer ${node.initializer?.name.text} - Caller ${caller}`)
      }
  ...
  function findParentCallerFunction(node) {
    ...
    if (ts.isFunctionLike(node) || ts.isMethodDeclaration(node))
      return node;
  ...
  function recordFunctionCallInContext(node, context, fileName) {
    ...
    if (!owner || owner == 'this' || owner == fileName || owner == 'this.props') {
      const store = checkFunctionFromImportedStore(callName, importMappings);
     
      if (store) {
        if (traceComponent && fileName == traceComponent) {
          logger.info(`Found store owner: %o`, store)
        }
        owner = store;
      } else {
        owner = fileName
  ...
  function processFunctionCalls(node, contextName = 'Global', fileName) {
    ...
    ts.forEachChild(node, child => processFunctionCalls(child, contextName, fileName));
  }
  ...
  function recordFunctionCall(node) {
    ...
    if (nodeName === 'anonymous') {
      if (traceComponent && fileName == traceComponent) {
        logger.info(`recordFunctionCall - Node identifier is: ${nodeName} - Parent is: ${node.parent.name?.text}`);
      }

      const parent = node.parent;

      if (parent && (ts.isMethodDeclaration(parent) || ts.isFunctionDeclaration(parent) || ts.isPropertyDeclaration(parent))) {
        nodeName = parent.name ? parent.name.getText() : 'anonymous';
        if (traceComponent && fileName == traceComponent) {
          logger.info(`recordFunctionCall - Falling back to parent identifier: ${nodeName}`);
        }
      }
  ...
  function processJsxElement(node) {
    ...
    (node.attributes?.properties || []).forEach(prop => {
      if (ts.isJsxAttribute(prop) && prop.initializer) {
        if (ts.isJsxExpression(prop.initializer) && prop.initializer.expression && ts.isCallExpression(prop.initializer.expression)) {
          let callName = prop.initializer.expression.expression.getText(sourceFile);
          let caller = 'JSX';
          let owner = fileName;
    ...
    if (ts.isJsxSelfClosingElement(node)) {
      ...
      const owner = importMappings[tagName] ? importMappings[tagName] : fileName;
  ...
  try {
    ...
    function visit(node) {
      if (ts.isClassDeclaration(node) && node.name) {
        ...
        if (node.decorators && node.decorators.some(decorator =>
...
function extractAPIs(filePath) {
  ...
  function visit(node) {
    ...
    } else if (ts.isMethodDeclaration(node) || ts.isFunctionDeclaration(node) || ts.isVariableDeclaration(node)) {
      functionName = node.name?.getText() || (node.initializer && node.initializer.name && node.initializer.name.getText());
  ...
  function extractEndpoint(node) {
    if (ts.isTemplateExpression(node) || ts.isNoSubstitutionTemplateLiteral(node)) {
      ...
      return ts.isStringLiteral(arg) || ts.isTemplateExpression(arg) || ts.isNoSubstitutionTemplateLiteral(arg) ? arg.getText() : undefined;
...
function isStoreFunction(functionName) {
  const stores = []; // it may be more than one exposing the same function

  if (functionName) {
    for (const store in storesFunctions) {
      // Check if the `functionName` exists in the array
      if (storesFunctions[store] && storesFunctions[store].includes(functionName)) {
        stores.push(store);
      }
    }
...
function checkFunctionFromImportedStore(functionName, imports) {
  let foundStore = null; 

  if (functionName) {
    
    for (let index = 0; index < storesFunctions.length; index++) {
      const store = storesFunctions[index];
      
      if (storesFunctions[store] && storesFunctions[store].includes(functionName)) {
        const importedStoreFunctions = Object.values(imports).filter(imp => storesFunctions[store].includes(imp));
        if (importedStoreFunctions.length > 0) {
          foundStore = importedStoreFunctions[0];
          break;
        }else if ((Object.values(imports).includes("HolidaysRootStore.ts") || Object.values(imports).includes("useStore.ts")) && rootStoreStores.includes(store)){
          foundStore = store;
          break;
        }
      }
  ...
  function visit(node) {
    ...
    if (ts.isTypeReferenceNode(node) && node.parent && ts.isPropertyDeclaration(node.parent)) {
      ...
      if(importMappings[type])
        rootStoreStores.push(importMappings[type]);
    }
  ...
  try {
    ...
    for (const filePath of files) {
      ...
      for (const imp of imports) {
        ...
        const importedFileType = getFileType(importedFileName, importedFiles[importedFileName] !== undefined ? importedFiles[importedFileName] : []);
        ...
        if (!skipImports.some(skpImp => skpImp === imp)) {
          ...
          logger.info(`${fileType}:${fileName} imports ${importedFileType}:${importedFileName}`);
        }
      }
    }
    ...
    for (const filePath of files) {
      ...
      for (const caller in functionCalls) {
        ...
        if (traceComponent && fileName == traceComponent) {
          ...
          logger.info(functionCalls[caller])
        }
        for (const calledFunc of functionCalls[caller]) {
          if (!excludedOwners.some(owner => calledFunc.owner.startsWith(owner))) {
            ...
            logger.info(`Function ${caller} in ${fileName} calls function ${calledFunc.callName} (${calledFunc.owner})`);
          } else {
...
createGraphData('../../codebase/frontend/app_/src').then(() => {
