...
function resolveModulePath(moduleName, filePath, baseDir) {
  let resolvedPath = '';
...
function getImportMappings(sourceFile) {
  ...
  ts.forEachChild(sourceFile, node => {
    if (ts.isImportDeclaration(node) && node.importClause) {
      ...
      if (node.importClause.name) {
        importMappings[node.importClause.name.text] = moduleName;
      } else if (namedBindings && ts.isNamespaceImport(namedBindings)) {
        importMappings[namedBindings.name.text] = moduleName;
      } else if (namedBindings && ts.isNamedImports(namedBindings)) {
        namedBindings.elements.forEach(spec => {
          const alias = spec.name.text;
          importMappings[alias] = moduleName;  // Keeping track of the module name directly
        });
      }
    }
  });
...
function extractFunctionCalls(fileName, filePath, baseDir) {
  ...
  if (traceComponent && fileName == traceComponent) {
    logger.info(`${fileName} - mobx injected methods: %o`, mobxInjectedStores)
  }
  ...
  function visit(node) {
    ...
    // }
    // Case1: Handle function declarations
    if (ts.isFunctionDeclaration(node) || ts.isMethodDeclaration(node) || ts.isFunctionExpression(node) || ts.isArrowFunction(node) || (ts.isVariableDeclaration(node) && node.initializer && ts.isFunctionLike(node.initializer))) {
      if (traceComponent && fileName == traceComponent && (node.name || node.initializer)) {
    ...
    //Case 6: Handle store function assignment
      ...
      ts.isPropertyAccessExpression(node.initializer) && checkFunctionFromImportedStore(node.initializer?.name?.text, importMappings, node.initializer?.expression?.name?.text)) {

      if (traceComponent && fileName == traceComponent && (node.name || node.initializer)) {
        logger.info(`Case 6: Handle store function assignment - Node ${node.name?.text} - Initializer ${node.initializer?.name.text}`)
      }

      const store = checkFunctionFromImportedStore(node.initializer?.name.text, importMappings, node.initializer?.expression?.name?.text);
  ...
  function findParentCallerFunction(node) {
    ...
    //findParentCallerFunction undefined - ArrowFunction
    //findParentCallerFunction loadData - PropertyDeclaration
  ...
  function recordFunctionCallInContext(node, context, fileName) {
    ...
    if (!owner || owner == 'this' || owner == 'this.props') {
...
function checkFunctionFromImportedStore(functionName, imports, parentObject) {
  ...
  if (functionName) {
    ...
    for (let index = 0; index < storeKeys.length; index++) {
      const store = storeKeys[index].split('.')[0];
      const storeFunctionArray = storesFunctions[storeKeys[index]];

      if (storeFunctionArray.includes(functionName)) { // a cazzo paystore dovrebbe essere paymentstore...stronzi
        ...
        if (importedStoreFunctions.length > 0) {
          ...
          break;
        } else if ((importedObjects.includes("frontend/store/holidays/HolidaysRootStore") 
          || importedObjects.includes("frontend/store/holidays")
          || importedObjects.includes("frontend/hooks/useStore")) 
          ...
          if (parentObject && 
            ...
            logger.info(`Parent object not found, falling back to store ${store}`)
            foundStore = store;
            break;
          }
        }
    ...
    for (const filePath of files) {
      ...
      for (const caller in functionCalls) {
        ...
        if (traceComponent && fileName == traceComponent) {
          ...
          logger.info(`%o`, functionCalls[caller])
        }
