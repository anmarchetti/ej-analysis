...
{
    public class RunLocationsSyncCommandTests : BaseSyncCommandTests
    {
        private readonly RunLocationsSyncCommand runLocationsSyncCommand;

        private readonly IDatabaseProvider databaseProvider;
        private readonly IUserCreationService userCreationService;
        private readonly ISitecoreUIService sitecoreUiService;

        public RunLocationsSyncCommandTests()
        {
            databaseProvider = Substitute.For<IDatabaseProvider>();
            userCreationService = Substitute.For<IUserCreationService>();
            sitecoreUiService = Substitute.For<ISitecoreUIService>();
            runLocationsSyncCommand = new RunLocationsSyncCommand(Service, Logger, databaseProvider, userCreationService, sitecoreUiService);
        }
        ...
        public void IsCommandContextValid_ShouldBeTrue_IfTempateIsValid()
        {
            ...
            resortDBItem.TemplateID = Constants.TemplateIds.Country;
            Db.Add(resortDBItem);

            var commandContext = new CommandContext(Db.GetItem(resortDBItem.ID));

            // Act
            var actual = runLocationsSyncCommand.IsCommandContextValid(commandContext);
        ...
        public void SynchronizeItems_ShouldBeEnumerableEmpty_IfCodeFieldHasNoValue(string code)
        {
            ...
            // Act
            var actual = runLocationsSyncCommand.ProcessItems(Db.GetItem(dbItem.ID));
