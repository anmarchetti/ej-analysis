...
{
    public class RunCountriesSyncCommandTests : BaseSyncCommandTests
    {
        private readonly RunCountriesSyncCommand runCountriesSyncCommand;

        private readonly IDatabaseProvider databaseProvider;
        private readonly IUserCreationService userCreationService;
        private readonly ISitecoreUIService sitecoreUiService;

        public RunCountriesSyncCommandTests()
        {
            databaseProvider = Substitute.For<IDatabaseProvider>();
            userCreationService = Substitute.For<IUserCreationService>();
            sitecoreUiService = Substitute.For<ISitecoreUIService>();
            runCountriesSyncCommand = new RunCountriesSyncCommand(Service, Logger, databaseProvider, userCreationService, sitecoreUiService);
        }
        ...
        {
            // Arrange
            var destinationDBItem = Fixture.Build<DbItem>().Without(x => x.ParentID).Create();
            destinationDBItem.TemplateID = Constants.TemplateIds.DestinationsFolder;
            Db.Add(destinationDBItem);

            var commandContext = new CommandContext(Db.GetItem(destinationDBItem.ID));

            // Act
            var actual = runCountriesSyncCommand.IsCommandContextValid(commandContext);
        ...
        {
            // Arrange
            Service.SyncCountries(Arg.Any<ID>(), Arg.Any<Item>()).Returns(new List<Item>() { item });

            // Act
            var actual = runCountriesSyncCommand.ProcessItems(item);
