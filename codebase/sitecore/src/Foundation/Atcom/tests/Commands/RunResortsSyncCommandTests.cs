...
{
    public class RunResortsSyncCommandTests : BaseSyncCommandTests
    {
        private readonly RunResortsSyncCommand runResortsSyncCommand;

        private readonly IDatabaseProvider databaseProvider;
        private readonly IUserCreationService userCreationService;
        private readonly ISitecoreUIService sitecoreUiService;

        public RunResortsSyncCommandTests()
        {
            databaseProvider = Substitute.For<IDatabaseProvider>();
            userCreationService = Substitute.For<IUserCreationService>();
            sitecoreUiService = Substitute.For<ISitecoreUIService>();
            runResortsSyncCommand = new RunResortsSyncCommand(Service, Logger, databaseProvider, userCreationService, sitecoreUiService);
        }
        ...
        {
            // Arrange
            var locationDBItem = Fixture.Build<DbItem>().Without(x => x.ParentID).Create();
            locationDBItem.TemplateID = Constants.TemplateIds.Location;
            Db.Add(locationDBItem);

            var commandContext = new CommandContext(Db.GetItem(locationDBItem.ID));

            // Act
            var actual = runResortsSyncCommand.IsCommandContextValid(commandContext);
        ...
        public void SynchronizeItems_ShouldBeEnumerableEmpty_IfCodeFieldHasNoValue(string code)
        {
            ...
            // Act
            var actual = runResortsSyncCommand.ProcessItems(Db.GetItem(dbItem.ID));
