...
{
    public class RunStarRatingsSyncCommand : BaseItemProgressReportingCommand
    {
        private readonly ISyncDataService service;

        public RunStarRatingsSyncCommand(
        ...
        {
            return context.Items[0].TemplateID.Equals(Constants.TemplateIds.StarRatingsFolder);
        }

        /// <inheritdoc />
        protected override IEnumerable<Item> ProcessItems(Item contextItem)
        {
            return service.SyncStarRatings(Constants.TemplateIds.StarRating, contextItem);
        }
    }
}