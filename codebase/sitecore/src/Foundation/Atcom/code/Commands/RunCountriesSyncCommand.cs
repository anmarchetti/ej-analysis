...
{
    public class RunCountriesSyncCommand : BaseItemProgressReportingCommand
    {
        private readonly ISyncDataService service;

        public RunCountriesSyncCommand(
        ...
        {
            return context.Items[0].TemplateID.Equals(Constants.TemplateIds.DestinationsFolder);
        }

        /// <inheritdoc />
        protected override IEnumerable<Item> ProcessItems(Item contextItem)
        {
            return service.SyncCountries(Constants.TemplateIds.Country, contextItem);
        }
    }
}