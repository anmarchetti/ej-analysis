...
{
    public class RunRoomFacilitiesSyncCommand : BaseItemProgressReportingCommand
    {
        private readonly ISyncDataService service;

        public RunRoomFacilitiesSyncCommand(
        ...
        {
            return context.Items[0].TemplateID.Equals(Constants.TemplateIds.RoomFacilitiesFolder);
        }

        /// <inheritdoc />
        protected override IEnumerable<Item> ProcessItems(Item contextItem)
        {
            return service.SyncRoomFacilities(Constants.TemplateIds.RoomFacility, contextItem);
        }
    }
}