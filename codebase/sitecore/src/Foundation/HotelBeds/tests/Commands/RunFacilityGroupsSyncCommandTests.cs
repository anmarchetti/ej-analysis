...
{
    public class RunFacilityGroupsSyncCommandTests : BaseSyncCommandTests
    {
        private readonly RunFacilityGroupsSyncCommand runFacilityGroupsSyncCommand;

        private readonly IDatabaseProvider databaseProvider;
        private readonly IUserCreationService userCreationService;
        private readonly ISitecoreUIService sitecoreUiService;

        public RunFacilityGroupsSyncCommandTests()
        {
            databaseProvider = Substitute.For<IDatabaseProvider>();
            userCreationService = Substitute.For<IUserCreationService>();
            sitecoreUiService = Substitute.For<ISitecoreUIService>();
            runFacilityGroupsSyncCommand = new RunFacilityGroupsSyncCommand(Service, Logger, databaseProvider, userCreationService, sitecoreUiService);
        }
        ...
        public void IsCommandContextValid_ShouldBeTrue_If_TempateIsValid()
        {
            ...
            itemDb.TemplateID = Destinations.Constants.TemplateIds.FacilityTypesFolder;
            Db.Add(itemDb);

            var commandContext = new CommandContext(Db.GetItem(itemDb.ID));

            // Act
            var actual = runFacilityGroupsSyncCommand.IsCommandContextValid(commandContext);
