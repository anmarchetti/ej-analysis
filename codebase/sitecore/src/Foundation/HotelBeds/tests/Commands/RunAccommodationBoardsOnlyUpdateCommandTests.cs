...
{
    public class RunAccommodationBoardsOnlyUpdateCommandTests : BaseSyncCommandTests
    {
        private readonly TestableRunAccommodationBoardsOnlyUpdateCommand sut;

        public RunAccommodationBoardsOnlyUpdateCommandTests()
        {
            ...
            sut = new TestableRunAccommodationBoardsOnlyUpdateCommand(Service, Logger, databaseProvider, userCreationService, sitecoreUiService);
        }
        ...
        [Fact]
        public void ProcessItems_ShouldSyncBoardsAndUpdateIndexes()
        {
            var hotelBedsCode = "HB-1";
            var contextItem = new FakeItem(ID.NewID)
                .WithTemplate(Destinations.Constants.TemplateIds.Accommodation)
                .WithField(Destinations.Constants.Fields.AccommodationItem.HotelBedsCode, hotelBedsCode)
                .ToSitecoreItem();

            var synced = new List<Item> { new FakeItem(ID.NewID).WithName("Board Item").ToSitecoreItem() };
            Service.SyncAccommodationBoards(hotelBedsCode, contextItem).Returns(synced);

            var result = sut.InvokeProcessItems(contextItem);

            result.Should().BeEquivalentTo(synced);
            Service.Received(1).SyncAccommodationBoards(hotelBedsCode, contextItem);
            Service.Received(1).UpdateMasterIndexes(contextItem);
        }

        private class TestableRunAccommodationBoardsOnlyUpdateCommand : RunAccommodationBoardsOnlyUpdateCommand
        {
            public TestableRunAccommodationBoardsOnlyUpdateCommand(
