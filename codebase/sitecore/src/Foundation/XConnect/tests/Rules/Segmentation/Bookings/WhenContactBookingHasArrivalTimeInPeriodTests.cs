...
{
    public class WhenContactBookingHasArrivalTimeInPeriodTests
    {
        private readonly IRuleExecutionContext ruleExecutionContext;
        private readonly WhenContactBookingHasArrivalTimeInPeriod predicate;

        public WhenContactBookingHasArrivalTimeInPeriodTests()
        {
            ruleExecutionContext = Substitute.For<IRuleExecutionContext>();
            predicate = new WhenContactBookingHasArrivalTimeInPeriod();
        }
        ...
        [Theory]
        [AutoData]
        public void Evaluate_ShouldBeTrue_IfHasArrivalInTimePeriod(Contact contact)
        {
            // Arrange
            predicate.StartOfPeriod = new DateTime(2024, 2, 10);
            predicate.EndOfPeriod = new DateTime(2024, 2, 15);
            var booking = new Booking()
            {
                Flights = new List<Flight>()
                {
                    new Flight()
                    {
                        ArrivalTime = new DateTime(2024, 2, 13)
                    }
                }
            };
        ...
        [Theory]
        [AutoData]
        public void Evaluate_ShouldBeFalse_IfHasArrivalInTimePeriod(Contact contact)
        {
            // Arrange
            predicate.StartOfPeriod = new DateTime(2024, 2, 10);
            predicate.EndOfPeriod = new DateTime(2024, 2, 15);
            var booking = new Booking()
            {
                Flights = new List<Flight>()
                {
                    new Flight()
                    {
                        ArrivalTime = new DateTime(2024, 3, 13)
                    }
                }
            };
