...
{
    public class OptimizelyVariantConditionTests
    {
        [Fact]
        public void Execute_ShouldReturnFalse_WhenExperimentationIsDisabled()
        {
            ...
            var sut = new TestableOptimizelyVariantCondition
            {
                FlagKey = "flag-a",
                Variant = "variation-a"
            };
        ...
        [Fact]
        public void Execute_ShouldReturnTrue_WhenEnabledAndVariantMatchesIgnoringCase()
        {
            var decisionService = Substitute.For<IOptimizelyService>();
            var gateService = Substitute.For<IOptimizelyExperimentationGateService>();
        ...
        [Fact]
        public void Execute_ShouldReturnFalse_WhenVariantDoesNotMatch()
        {
            var decisionService = Substitute.For<IOptimizelyService>();
            var gateService = Substitute.For<IOptimizelyExperimentationGateService>();
            gateService.IsEnabledForCurrentLanguage().Returns(true);
            decisionService.Decide("flag-a", OptimizelyDecisionSource.ComponentPersonalization)
                .Returns((true, "variation-b", (IDictionary<string, object>)new Dictionary<string, object>()));
        ...
        private sealed class TestableOptimizelyVariantCondition : OptimizelyVariantCondition<RuleContext>
        {
