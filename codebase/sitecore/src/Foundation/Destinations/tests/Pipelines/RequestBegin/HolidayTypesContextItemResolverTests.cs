...
{
    public class HolidayTypesContextItemResolverTests
    {
        private readonly HolidayTypesContextItemResolver resolver;
        ...
        public HolidayTypesContextItemResolverTests()
        {
            var itemResolver = Substitute.For<IItemResolver>();
            routeMapper = Substitute.For<IRouteMapper>();
            cache = Substitute.For<HtmlCacheRepository>();

            // Arrange
            fixture = new Fixture();
            db = fixture.Freeze<Db>();
            resolver = new HolidayTypesContextItemResolver(itemResolver, routeMapper, cache);
        ...
        public void Resolve_ShouldBeNull_IfPathDoesntStartWithPrefix()
        {
            ...
            args.PageContext.RequestContext.HttpContext =
                new HttpContextWrapper(new HttpContext(new HttpRequest("fake", "http://sc.holidays.local", "item=test"), new HttpResponse(null)));
        ...
        {
            // Arrange
            var expectedPathPrefix = "/Holiday Types";

            var fullPath = $"{expectedPathPrefix}/{itemPath}";
            var fakeSite = new FakeSiteContext("Site");
            var cacheKey = $"HolidayTypeItem-{fullPath}";
            ...
            using (new FakeSiteContextSwitcher(fakeSite))
            using (new SettingsSwitcher("Destinations.HolidayTypes.JssApiPrefix", expectedPathPrefix))
            using (new SafeContextItemSwitcher(null))
            {
                // Act
                resolver.Process(args);
