...
#Columns: Code, ExpirationDate
$vouchers = import-csv ".\output.csv"

Write-Output "`n`nUpdating expiration date (rollback):"
$i = 1
foreach($row in $vouchers) {    
    Write-Output "$($row.Code),$($row.ExpirationDate)"

    $expDate = ([DateTime]$row.ExpirationDate).ToUniversalTime()
    # extend expiration by one year
    $expDate = $expDate.AddYears(-1)
   
    try {
        $response = (updateExpirationDate $row.Code $expDate) 		
        Write-Output "$i $($response.code),$($response.expiration_date)"
    } catch {
        Write-Output "ERROR: Cannot update $($row.Code): $_"
    } 
    $i++       
}
