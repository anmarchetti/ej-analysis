...
import generateComponentFactory, { IComponentFile } from './templates/component-factory';
...
const rootPath = 'src';
const componentFactoryPath = path.resolve(rootPath, 'temp/componentFactory.ts');
const componentsPath = 'frontend/components/renderings';
const componentRootPath = path.join(rootPath, componentsPath);
const skipFolders = ['components', 'models', 'store', 'data', '__tests__', '__mocks__', 'iframe', 'hooks'];

// list of abbreviations used in component names in sitecore
const wholeWords = ['ATOL', 'IE', 'FAQ', 'SideBar', 'SEO'];

// Exception components that don't follow the naming convention.
// Object type whose property key is React component name, property value is Sitecore rendering name.
// (e.g., TestComponent: 'Test 2 Component')
...
function getComponentList(pathStr: string): IComponentFile[] {
    const components: IComponentFile[] = [];
    const folders: fs.Dirent[] = [];

    try {
        const relPath = path.relative(rootPath, pathStr).replace(/\\/g, '/');

        fs.readdirSync(pathStr, { withFileTypes: true }).forEach(item => {
            // do not check components in inner "components folder"
            if (item.isDirectory() && !skipFolders.includes(item.name)) {
                folders.push(item);
            }
            ...
            if (fileFormat.test(item.name)) {
                ...
                const nameRegex = new RegExp(`${wholeWords.join('|')}|[A-Z][a-z]+|[0-9]+`, 'g');

                let componentName = '';

                if (exceptions[name]) {
                    componentName = exceptions[name];
                } else {
                    componentName = (name.match(nameRegex) ?? name.split(/(?=[A-Z])/)).join(' ');
                }
        ...
        for (const folder of folders) {
            components.push(...getComponentList(`${pathStr}/${folder.name}`));
        }
    } catch (e) {
        console.log(e);
    }
