...
module.exports = (req, res) => {
  ...
  try {
    const filePath = path.join(process.cwd(), 'data', 'layout-positions.json');
    ...
    return res.status(200).json({ status: 'success', message: 'Layout saved' });
  } catch (err) {
    console.error('Failed to write layout file:', err);
